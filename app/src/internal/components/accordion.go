...
func AccordionItemComponent(index int, item AccordionItem) g.Node {
	...
	detailsAttrs = append(detailsAttrs,
		Class("accordion-item border-b border-foreground last:border-b-0"),
	)
	if isFirst {
	...
	return g.El("details",
		...
		g.El("summary",
			Class("accordion-trigger w-full text-left text-sm md:text-base font-light hover:text-foreground py-3 px-4 cursor-pointer text-foreground flex justify-between items-center list-none"),
		...
		// Content
		Div(
			Class("accordion-content text-foreground text-xs md:text-sm leading-relaxed font-light"),
			Div(
				Class("px-4 py-3"),
