...
func setup_network(ctx *pulumi.Context, cfg *Config) (*Network, error) {
	...
	gateway, err := core.NewInternetGateway(ctx, "me-ig", &core.InternetGatewayArgs{
		CompartmentId: pulumi.String(cfg.Compartment),
		VcnId:         vcn.ID(),
	...
	routes, err := core.NewRouteTable(ctx, "me-rt", &core.RouteTableArgs{
		CompartmentId: pulumi.String(cfg.Compartment),
		VcnId:         vcn.ID(),
	...
	security, err := new_security(ctx, cfg, vcn)
	if err != nil {
		return nil, err
	}
...
func new_security(ctx *pulumi.Context, cfg *Config, vcn *core.Vcn) (*core.SecurityList, error) {
